...
- CCSIM backlog —— whilst drafting the close, scan THIS session for anything CCSIM-worthy
  - inc. but not limited to: script/pcmd/"skill" fix/improvement, recurring snag, system gap
  - e.g. a query should've triggered [X] skill → you didn't read [X].md → user reminded by #[X]
...
- §SHA MUST be the FINAL section of every `close_` (DevPlan or not)
  ...
  - Obtain via `git log --pretty=%h --abbrev=8 [1st_session_SHA]^..HEAD` (per repo)
    - Caveat: no marker separates a parallel session's commits, so the range may hold them
    - Own `🦈` lines are authoritative —— a SHA only the range shows is NOT yours
