...
A distilled, CC-facing guideline to Culous Yu / C.YU MGT's personal visual identity —— colour, type, logo, and layout conventions for consistent design/visual output (HTML, slides, documents, decks). C.YU MGT is not a live agency; Culous no longer runs it as a real firm, so treat this as a personal identity, not corporate brand governance —— keep design copy in a personal-portfolio register, never "our company/our team" language.
...
- Text-colour pairing applies across all 4 fonts, not just headings:
  ...
  - `#FFFFFF` —— ~99.9% of white-on-dark-background text (e.g. on Dark Teal).
...
- Opacity is size-dependent by usual practice —— full/high opacity when small, half/low opacity when large —— judged for visual balance case-by-case; culousyu.com deliberately breaks this (large AND full opacity) because that context is "100% about" Culous, highly stylised and self-focused —— don't assume that exception generalises to other outputs.
- Appears two ways in practice: a small solid-line badge icon (page corners/headers, full opacity) or a large translucent "ghost" watermark over photography (cover/closing slides, low opacity) —— never as a solid-fill logo.
...
For linguistic register/tone (email, DM, formality level), see `writing.md` —— this section is design-layer emphasis only, not a duplicate of that file.

- Inline keyword-highlighting in Mid Teal within body paragraphs draws the eye to key achievements or links.
- Quantified-achievement emphasis (%, ×-multiples, counts, `+`) shows up specifically in client/employer-facing copy (management-consulting audiences expect it) —— NOT a personal trait; Culous doesn't otherwise think in numbers this way, so don't generalise it beyond that audience.
...
- Fine-art photography —— awarded by HIPA, PX3, WPO, etc.; published by Vogue, NatGeo, Sony, etc.; a distinct creative practice documented only on culousyu.com (ARTWORKS section), not in the business portfolio. Visually its own register, not the teal business palette: moody monochrome/black-and-white, architectural and landscape/nature subjects (e.g. a lone sailboat, a symmetric corridor, a hazy shoreline). The Signature Motif's "Miller" is itself one of these award-winning shots, just in colour and repurposed as chrome —— treat this facet's own imagery as B&W-first.
