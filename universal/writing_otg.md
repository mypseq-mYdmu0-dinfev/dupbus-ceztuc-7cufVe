# Writing Rules (non-CC)

## Shared Rules (all writing styles)
...
- Avoid english numbers (except "one"), use numerals instead (easier human reading):
  ...
  - Except in Casual Writing (e.g. "3rd"), use an english number for ordinals (e.g. "third")
...
- Prefer precise, stronger wording over weak/unsophisticated words
  ...
  - Actively avoid "want", "something", and "big" specifically, though this is a preference to weigh by context (e.g. Casual Writing), not an absolute ban
...
- Be extra "human", with a friendlier tone in simple wording
- Avoid periods `.`; just start a new line instead
- When told "convert to whatsapp/WA":
  - Bold: **text** → *text*
  - Italic: *text* → _text_
  - Leave the rest unchanged (monospace ``, list -, quote >, etc.)
