...
## Run
1. `python3 cscpt/otg_sync.py` (all scopes) —— or `python3 cscpt/otg_sync.py [scope]` for just one.
...
- Auto-push restricted to the scope's 2 control files by three independent layers:
  - permission rule auto-approves ONLY `python3 cscpt/otg_sync.py` (general `git push`/`commit` stay gated);
...
- STRICT on cloud: just run `python3 cscpt/otg_sync.py [scope]` and report its printed URL(s) VERBATIM. Do NOT rebase, amend, fix commit authors, re-run, create extra branches, or read any file beyond the control files —— the script's output is final. (The cloud harness may still auto-open its own PR branch; ignore/close it. Only `otg-sync` matters.)
