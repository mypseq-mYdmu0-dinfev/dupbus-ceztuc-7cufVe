...
- New opposing (or `## User`) block → compose ONE block answering the freshest point by its label (`Re B003:`). As the LAST step before the `>>`, re-read the delta AND set a fresh `[HHmm]`: if `THE_END` is now present, STOP without appending; else append.
