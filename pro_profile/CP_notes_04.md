...
## Purpose
This CP ("Professional Profile") is for anything related to my work: resume, cover letter (CL), interview, pitch, etc. Always use confident language & powerful proactive verbs to highlight relevant skills/experience in deliverables. Never use "I believe/feel/think".
...
- Start by opening & reading full job post on Line 3 `URL:` via ACIC; stop if failed (no fallback)
- In 1st artefact, critically evaluate PAR after full read; concisely tell if further research needed
- If research needed, follow cic.md using web_search + ACIC
- If my msg doesn't indicate CCL/GCL, tell at 1st artefact end which protocol suits
- In 2nd artefact, follow CCL/GCL instr

## Remarks
- Assume CAI by default in this CP. If ACIC access fails, stop & remind to use CAI.
