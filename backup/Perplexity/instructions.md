...
Start with `**Full Text Parsed**: ✅/❌` by **fetch_url** on exact URL
- **Return `✅` if:**
  ...
  - you can reliably extract **both first and last words** (`culousyu.com` is a `✅` example).
- **Return `❌` if:**
...
- If and only if `✅`, print first/last 10 words then proceed to summarise only the site’s own content, never fabricate or insert external content.
- If `❌`: Tell approx word count of result from fetch_url alone (not search_web), if and only if it’s 100+, proceed accordingly.
- If I send text (100+ words) after `❌`, consider it full text and run `FTS` again

---

# Claude's Weekly Limit (CWL)

When I prompt `CWL`, apply this prompt for the provided data [weekday] [time] [no.]%` (might shuffle order) which is my usage limit with Claude AI. Q: quota usage; T: time elapsed in weekly session.
