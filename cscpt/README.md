...
**Inside a file, the funnel is: headline → `NON-CCSIM` → `CCSIM` → full code (rare).** Line 2 of every script is a one-line headline —— full name first, trigger context in trailing parentheses —— whose sole job is saying WHICH script a feature belongs in; the user quick-peeks by it, so depth stays in the blocks below.
