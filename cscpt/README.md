...
- `dlint_hook.sh` —— PostToolUse fast-path shim wired in root `.claude/settings.json`. Exits 0 instantly unless the payload mentions `response_` (so Python is spawned only when relevant), then delegates to `dlint_hook.py`. **Run by the harness, not read.**
- `dlint_hook.py` —— the hook body. On a `response_*.md` write/edit it runs `dlint.py --quick` and blocks (exit 2) until 🔴 RED = 0. Fires ONLY on `response_` files (never code/automations/`/seek/`); fail-safe (exits 0 on any error). **Run by the harness, not read.**
