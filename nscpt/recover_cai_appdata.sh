...
final_guidance(){
  ...
  case "$INTEGRITY" in
    ...
    fail)
      ...
      say ""
      say "  RESTORE — the corrupt data lives on FURY (that's where CAI runs from"
      say "  after migration), so the restore TARGET is the FURY path, NOT the"
      say "  internal one (~/Library/... is only a symlink to it):"
      say "    A. IF FURY is in your Time Machine backup set: open Time Machine,"
      say "       restore ONLY this ONE folder (not the whole Mac/disk):"
      say "           /Volumes/FURY 2TB/Library/Application Support/Claude"
      say "       to the LATEST snapshot you are SURE was good (before the event)."
      say "    B. IF FURY is NOT in Time Machine (external volumes are EXCLUDED by"
      say "       default): TM can't help the FURY copy. Instead use the internal"
      say "       *.premigrate-backup-* copy (migration-time snapshot), or just"
      say "       relaunch CAI to re-sync sessions from the cloud."
      say "    Then relaunch CAI (FURY mounted)."
      say ""
