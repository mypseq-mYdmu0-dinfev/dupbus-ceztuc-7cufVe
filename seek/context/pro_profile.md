# Pro Profile — Culous Yu

## Personal

**Name:** Culous Yu
**Location:** Sydney, NSW —— applying to Sydney roles only
**Visa:** Subclass 485, full work rights until 2031; never requires visa sponsorship
**Languages:** English (IELTS 8.0, Speaking 9.0), Cantonese, Mandarin
**Career start:** 2010, England

---

## Education

- **Master of Human-Computer Interaction** (sub-major: Data Analytics), UTS —— graduated NOV 2025, HD (88%), Dean's List, Australian AI Institute recognition for data visualisation project
- **MBA**, University of Liverpool —— final dissertation stage; expected graduation late 2026
- **Higher Diploma in Cinematic Arts**, HK Baptist University
- **McKinsey Forward Programme** (completed, 10-week, OCT-DEC 2025)
- **Apple Foundation Programme** (completed, MAY-JUN 2025)

---

## Career Summary

16⁺ years international experience across B2B consultancy, B2C brand and media, NPO, and government. Career-wide personal records (across all firms, not solely attributable to KE): 300⁺ projects, 100⁺ clients, 10⁺ countries.

**Key Strengths:** Strategic Transformation, Value Engineering, Stakeholder Management

---

## Work History

- **Board Member & Advisor (Strategy & Operations)**, KE (Karma Effect Ltd.) —— Jun 2025-present (pro bono, post-M&A)
- **M&A PMO**, KE —— Jan 2025-Jun 2025
- **Remote General Manager**, KE —— Jul 2023-Jan 2025
- **General Manager**, KE —— Apr 2020-Jul 2023 (grossed $1 million⁺ in first fiscal year as a startup consulting firm; recipient of Hong Kong's Most Outstanding Business Awards 2022)
- **Assistant Marketing Manager**, HK Equestrian Federation —— Aug-Oct 2023 (participated in 19th Asian Games; organised FEI (Fédération Equestre Internationale) World Challenge 2023 HK; directed rebranding for 50th Anniversary)
- **Visual Director**, Backbone Limited —— Nov 2018-Apr 2020

**KE Designation Rules:**
- Never disclose that Culous founded KE
- For junior roles: downplay bg, only mention Remote GM designation
- KE M&A engagement (Jan-Jun 2025): acquirer was also Hong Kong-based —— never describe as cross-border
- Revenue phrasing: "led the teams to $1 million⁺" (always plain "$", never "HK$")

---

## Notable Credentials
⚠️ Only reference items below when highly relevant to the role. These are retained as writing material, not default inclusions.

- iOS app developer: SpeakMate (GenAI/ML integration)
- UTS "Advanced Data Analytics Algorithms" subject: gold price prediction ML model (R² = 0.9999) —— this is the ML model's sole achievement
- UTS data visualisation project: selected by AAII as academic benchmark —— this is the AAII recognition's sole attribution; never conflate with ML model
- Photography: published by National Geographic, Vogue, Sony; awarded by HIPA, Px3, World Photography Organisation
- Film: 2018 Golden Sugarcane Film Festival —— second place overall, Best Cinematography
- UTS FEIT iMentor Programme (completed Feb-Apr 2025)

---

## Resume Decision Rules

| Role Type | Resume Filename |
|---|---|
| Marketing, Branding, PR, Social Media, Content | `Culous_Yu_Resume_Marketing.pdf` |
| UX/UI Design, Service Design, Product Management, Digital Product | `Culous_Yu_Resume_IxD.pdf` |
| IT, IS, Data Analytics, Software Engineering, Technical | `Culous_Yu_Resume_IT.pdf` |
| All others (Admin, Operations, Customer Service, Office Management, etc.) | `Culous_Yu_Resume_General.pdf` |
| Consulting, Strategy, Advisory (any) | **SKIP —— Save job on SEEK, do not apply** |

**`Culous_Yu_Resume_Consulting.pdf` must never be selected in this workflow.**

---

## Explicit Rules for Cover Letters

- **P.S. line:** as defined in `gcl.md` —— do not hardcode here; gcl.md is the single source of truth
- No text after the P.S. line in any cover letter —— ever
- No dash `—` or colon `:` in cover letter body
- Always "Yours Faithfully" (not "Sincerely") since recipient's name is unknown
- Never use "SQL" —— Culous has no SQL knowledge
- "HCI" first use as "Human-Computer Interaction (HCI)", unless role is design-centric (use "Interaction Design (IxD)"; thereafter "HCI"/"IxD" is acceptable
- Always "Advanced Data Analytics Algorithms" (never "Advanced Data Analytics") when referencing that UTS subject
- Career-wide figures (16⁺ years, 300⁺ projects, 100⁺ clients, 10⁺ countries) must each appear no more than once per cover letter
- Virtual Presentation Script: include `http://Presentation.CulousYu.com` as plain text URL in cover letter body if highly applicable
- `Portfolio.CulousYu.com` = full PDF portfolio (more detailed); `Projects.CulousYu.com` = website (more concise); same content, different form —— use Portfolio.CulousYu.com for design/creative roles, Projects.CulousYu.com for other roles
- Don't use abbrev. (e.g. `KE`, `ML` must be in full) unless prevalent (e.g. `AI`, `IT`)

---

## Key Links

- Portfolio / Case Studies: http://Projects.CulousYu.com
- Virtual Presentation: http://Presentation.CulousYu.com
- KE website: https://www.KarmaEffect.co