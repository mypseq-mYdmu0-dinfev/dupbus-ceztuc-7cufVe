# TPS in KBFs — Concept Brief (Private/Concise)

---

## Glossary

- TPS — Tall Poppy Syndrome
- TP — Tall Poppy (high performer who stands out and faces social friction; = HP)
- TPP — TP's Peers (colleagues who experience/witness TPS dynamics; primary ppts)
- KBF — Knowledge-Based Firms (e.g. consulting, IT, creative, advisory, R&D; see KBV)
- KBV — Knowledge-Based View (extends Barney's RBV)
- KM — Knowledge Management
- RQ — Research Question
- DA — Dissertation Advisor
- Q — Qualitative | N — Quantitative
- RQ1`~`RQ5 — RQ options (see Section 7)
- ppt — participant | mgr — manager | org — organisation | bg — background

---

1. **Topic**
- 1.1. Title — "Tall Poppy Syndrome as a Management Challenge in Knowledge-Based Firms"
- 1.2. One-liner — How TP dynamics create team cohesion and KM governance risks in KBFs, and what mgrs can do about it
- 1.3. Industry-wide scope — TPS is supra-organisational by nature; studying it in one firm risks conflating findings w/ that firm's specific culture/leadership idiosyncrasies, making it impossible to isolate TPS itself; cross-firm sampling = methodological necessity, not a workaround
- 1.4. Researcher positionality — Culous is a TP; first-hand experience provides reflexivity material for methodology chapter (strength, not conflict)

2. **Management Challenge**
- 2.1. Primary — Challenge A: team cohesion erosion + KM governance risk
  - When TP's output far exceeds team norm, peers disengage, collaboration declines, morale suffers
  - TP marginalised socially → logically withdraws from KM → converts interpersonal tension into boardroom-level KM governance problem (individual vs. collective success)
  - Mgr dilemma: penalising excellence vs. permitting mediocrity; no clean solution → research value
  - This is NOT a psychological study; focus is on organisational consequence and managerial response
- 2.2. Secondary — Challenge B: TP attrition cost
  - Sustained social exclusion → TP disengages/leaves → measurable HR/financial cost
  - Quantifiable angle; useful supplement in A2/A3 even if A is primary
- 2.3. Tertiary — Challenge C: systemic conditions enabling TPS
  - Reward structures, leadership modelling, cultural norms that allow TPS to persist
  - Boardroom-level cultural problem; elevates scope
  - Too broad for single-researcher project as primary; better as discussion/recommendation layer
- 2.4. Excluded — Challenge D: TP personal wellbeing; ethics risk (sensitive/distress territory)

3. **Why Original**
- 3.1. TPS literature dominated by cultural psychology (AU, NZ focus) and sociological context; management application in KBFs largely absent
- 3.2. Most org behaviour research on TPs assumes net-positive outcome; hidden costs to team and KM are underexplored
- 3.3. KM angle distinguishes this from HR/psychology territory and aligns w/ KBV literature and Enis' research expertise
- 3.4. Timeless problem (unlike AI-specific topics); findings won't expire

4. **Research Perspective & Participant Rationale**
- 4.1. Primary subjects — TPPs (non-mgr workers in KBFs who've worked alongside a TP and experienced TPS dynamics)
  - Rationale — TPPs are proximal experiencers of TPS; their accounts surface root causes and day-to-day blockers invisible to mgrs; this is fuelling practitioners, not competing w/ them
  - TPPs also more willing to speak (workplace hierarchy doesn't apply cross-firm, and anonymity is protected)
- 4.2. Supplementary — TP + mgr as triangulation/member-checking if secured; NOT to be pre-committed in proposal; present as bonus if secured before analysis
- 4.3. Recruitment criterion — experiential: "worked alongside a standout performer and experienced friction or tension in that context"
  - Note: recruited TPPs may be TPs themselves (high achievers know high achievers); this enriches rather than contaminates data; 2 question guides to accommodate both
- 4.4. Stakeholder matrix

| Stakeholder | Role | Interview guide | Required |
|---|---|---|---|
| TPP | Primary analysis | Guide A | Yes (6 min.) |
| TP | Triangulation | Guide B | Bonus if secured |
| Mgr | Triangulation | Guide C | Bonus if secured |

5. **Methodology**
- 5.1. Primary — Q; semi-structured interviews; 6 TPPs (default min.); expand if TP + mgr secured
- 5.2. Alternative — N; survey; 30+ ppts; open if DA steers towards quant; Culous bg = comfortable w/ either
- 5.3. Analysis — thematic analysis (Braun & Clarke 2006); NVivo optional (manual also accepted per Enis)
- 5.4. Multi-org design — 1 ppt per org = no org-level consent letter needed (per Enis, Wk 1&3 seminars)
- 5.5. Philosophy — interpretivist; inductive; no cause-and-effect language (RQs start w/ "what"/"how" only, per #25)

6. **Feasibility**
- 6.1. Participant access — TPP recruitment via Culous' professional network across 10⁺ countries (consulting, entertainment, FMCG, hospitality, NGOs, govt)
- 6.2. Multi-org = 1-per-org design; no consent bottleneck
- 6.3. Anonymisation — org name anonymised; industry stated (per Enis Wk 2); cross-firm design further protects identity
- 6.4. Dual question guide = standard in qual research; ethics-safe if framed at professional dynamics level (not personal emotional disclosure)

7. **RQ Options**

| Label | RQ | Stars | Note |
|---|---|---|---|
| RQ1 | How do mgrs in KBFs recognise and respond to team-level tensions arising from TP dynamics? | ★★★★ | Locks into mgr interviews; feasibility risk |
| RQ2 | What managerial strategies in KBF teams most effectively balance individual TP excellence w/ collective cohesion? | ★★★ | Predetermines answer ("balance"); risky conclusion |
| RQ3 | How do KBF team members and mgrs perceive TP dynamics, and what responses do they employ? | ★★★ | Too descriptive; lacks prescriptive depth for A-grade |
| RQ4 | What conditions within KBFs exacerbate or alleviate TPS, and how do team members experience them? | ★★★★★ | Best direction; phenomenon-level; peer-centric; evaluative/explanatory; prescriptive endpoint natural |
| RQ5 | How do TPs in KBFs experience team dynamics, and what support do their mgrs provide? | ★★ | Psychology-heavy; ethics risk |
- 7.1. Working direction — RQ4-style; final wording TBD after DA input
- 7.2. Sub-RQs to be derived from lit review (per Enis' golden thread rule)

8. **DA Ranking**

| Rank | DA | Key fit | Note |
|---|---|---|---|
| #1 | Enis Elezi | KM, org behaviour, teamwork, HRM/HRD, change mgt | Tight fit across all challenge angles; knows Culous |
| #2 | Hoang Thi Phuong Tra | Org behaviour, HRM, sense of responsibility | Solid alt if Enis overloaded |
| #3 | Dotun Adebanjo | HRM, broad coverage, all methods | Safe experienced pair of hands |
- 8.1. If quant angle taken — Nasir Salari may outperform above (marketing analytics/consumer behaviour fit less relevant here; Adebanjo still better)
- 8.2. DA role — calibrate nuances (RQ wording, scope, methodology); not expected to redirect topic fundamentals; final challenge angle and RQ may shift slightly per DA preference

9. **Risk Register**

| Risk | Mitigation |
|---|---|
| Thematic complexity if combination design | Present 6-TPP as default; triangulation as bonus only if secured |
| Literature thin on TPS in mgmt/KBF context | This IS the gap — frame as the contribution, not a weakness |
| Qual findings generalisability challenged | Acknowledge as limitation; cross-firm design partially addresses |
| Ethics: emotional/distress disclosure | Question framing stays at professional team dynamics level throughout |
| 8k word limit + multi-angle scope | Commit to Challenge A as primary; B&C as discussion layer only |
| TP recruitment (hard to find) | TPP-first design bypasses this; TP as bonus |
| Marker may question why not single firm | Industry-wide rationale via supra-organisational phenomenon argument (Section 1.3) |