...
- The skill listing —— every installed skill's `name` plus `description` —— is budget-capped at **30,000 characters**, and that cap is shared by project skills, plugin skills, and built-ins alike. Descriptions therefore COMPETE: every character one spends is a character another cannot have, and an over-budget listing gets shortened rather than negotiated. Count CHARACTERS throughout —— characters are the enforced quantity, and quoting a token-equivalent against a character cap invites a 3–4× arithmetic error.
- **30,000 is derived, not a constant.** The budget is 1% of the context window in character-equivalent terms: a 1,000,000-token window, converted at 3 characters per token, gives 30,000. A future model with a different window therefore moves the figure —— recompute it as `context_window_tokens × 0.01 × 3` rather than trusting the constant above, which is only as current as the window it was derived from. The 1,536-character per-description cap (below) is a separate flat setting, not a fraction of the window, so it does NOT move with it.
...
- **Over budget, the harness shortens rather than asks.** Two limits bite independently: `skillListingMaxDescChars` (default 1,536 characters) truncates any single over-long description, whilst `skillListingBudgetFraction` (default `0.01`) sets the shared listing budget —— the 1% that yields the 30,000 characters above. Once the total exceeds it, descriptions are mechanically shortened to fit, with no warning and no prompt to choose what goes, and in the worst case skills drop out of the listing altogether. Both are raisable in `settings.json`, but raising them only relocates the cost onto every turn; it never removes it.
...
- Measure, don't guess —— and measure in the ENFORCED unit: concatenate every `name: description` line into one file and run `LC_ALL=en_US.UTF-8 wc -m <path>`. Characters, not bytes: `wc -c` overcounts, because an em dash is one character but three bytes. Measured at the time of writing: the 16 project skills = 4,627 characters. That is the project's contribution ALONE —— plugin and built-in skills draw on the same 30,000 and are not counted in it, so re-measure against the live budget rather than assuming the headroom this figure implies.
- Working target: **≤300 characters** per description —— a self-imposed target, well under the 1,536-character hard cap, which truncates instead of guiding. Past ~400 you are crowding a sibling out, so spend the extra length only where it demonstrably buys matchability (see §3).
