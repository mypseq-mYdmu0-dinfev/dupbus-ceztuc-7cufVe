...
- 8.5. CCSIM's bar is HIGHER than elsewhere: an error made here is inherited by every other CC, so a wrong default ships silently to all of them and is found by whoever trips over it, not by you. The recurring way that bar is missed: mistaking your OWN manual verification for evidence that the SYSTEM works. Running a script by hand proves the SCRIPT and nothing else —— never the wiring that was supposed to run it. Verify through the REAL path (let the hook/trigger/session-start fire it unaided) and treat "it passed when I ran it myself" as UNPROVEN.
- 8.6. Never state a conclusion more broadly than its evidence supports —— one verified case licenses a claim about that case, not about the class. If the wider claim is the one that matters, go and test it.
- 8.7. When a rule WAS broken, diagnose why before fixing: NOT-NOTICED vs noticed-but-misapplied. Not-noticed is an enforcement gap, and the prose that failed cannot repair it —— bolding, repeating, or relocating words that were already skipped changes nothing while feeling like a fix. Ship a lint/hook/default that surfaces the rule at the moment of the act, or state plainly that the miss stays possible. Only a MISAPPLIED rule is fixed by rewriting the words.